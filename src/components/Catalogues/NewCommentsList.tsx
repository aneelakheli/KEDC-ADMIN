import { getNewComments, updateCommentStatus } from '@/serivces/catalogueService';
...
function NewCommentsList({ catalogueId }: { catalogueId: String }) {
    const queryClient = useQueryClient();

    const { data: commentData, isLoading, error, isError } = useQuery({
        queryKey: ['newComments', catalogueId],
        queryFn: () => getNewComments(catalogueId),
    });
    ...
    const toggleApprovalMutation = useMutation({
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries(['newComments', catalogueId]);
            console.log(data);
        },
        onError: (err) => {
    ...
    console.log("New Comments",commentData, commentData?.data?.length>0)

    return (
        <div className="p-6 bg-white rounded-lg shadow-lg dark:bg-gray-800 mt-4">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">New Comments</h2>
            {isLoading ? (
                <div
